        ...
        <div class="col-md-9 add-punterbox-report">
            <form id="punterbox-reports">
                <div class="form-group">
                    ...
                    <input type="hidden" name="id" value="{{$punterbox ? $punterbox->id : ''}}">
                    
                    <input type="date" class="form-control" name="incident_date"   value="{{ $punterbox && $punterbox->incident_date ? \Carbon\Carbon::parse($punterbox->incident_date)->format('Y-m-d') : '' }}" required>
                </div>
                <div class="form-group">
                    ...
                    <select class="custom-select" name="incident_state" required>
                        ...
                        @php
                        $st = $punterbox ? $punterbox->incident_state : null;
                        if($st == null){
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="Which city were you in" value="{{$punterbox ? $punterbox->incident_location : ''}}" name="incident_location" required>
                </div>

                <div class="form-group">
                    <label>Escort's Name</label>
                    <input type="text" class="form-control" placeholder="If known" value="{{$punterbox ? $punterbox->escorts_name : ''}}" name="escorts_name">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="email" class="form-control" value="{{$punterbox ? $punterbox->escorts_email : ''}}" name="escorts_email" placeholder="If known">
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" value="{{$punterbox ? $punterbox->platform : ''}}" placeholder="If known" name="platform">
                </div>

                <div class="form-group">
                    <label>Profile Link</label>
                    <input type="text" class="form-control" value="{{$punterbox ? $punterbox->profile_link : ''}}" name="profile_link" placeholder="Link or Membership ID or Ref">
                </div>

                <div class="form-group">
                    <label class="required">What Happened</label>
                    <textarea class="form-control" rows="4"  name="what_happened" required>{{$punterbox ? $punterbox->what_happened : ''}}</textarea>
                </div>

                <div class="form-group">
                    <label class="required d-block">Rating</label>
                    <div class="form-check d-flex align-items-center">
                        <input class="form-check-input" {{($punterbox && $punterbox->rating == 'Do Not Book') ? 'checked' : ''}} value="Do not book" checked type="radio" name="rating" id="rate1">
                        <label class="form-check-label" for="rate1">Do not book</label>
                    </div>
                    <div class="form-check d-flex align-items-center">
                        <input class="form-check-input" {{($punterbox && $punterbox->rating == 'Exercise caution') ? 'checked' : ''}} value="Exercise caution" type="radio" name="rating" id="rate2">
                        <label class="form-check-label" for="rate2">Exercise caution</label>
                    </div>
                    <div class="form-check d-flex align-items-center">
                        <input class="form-check-input"  {{($punterbox && $punterbox->rating == 'Safe') ? 'checked' : ''}} value="Safe" type="radio" name="rating" id="rate3">
...
<script>
    ...
    $("#punterbox-reports").on('submit', function(e){
        ...
        if (form) {
            ...
            var url = "{{route('user.update-my-reports')}}";
