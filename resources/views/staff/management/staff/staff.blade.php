        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content basic-modal">
                ...
                <div class="modal-body">
                    <form name="add_staff" id="add_staff" method="POST" action="{{ route('staff.add-staff') }}"
    ...
    <script>
        $(document).ready(function() {
            var table = $("#staff_data_table").DataTable({
                ...
                bStateSave: false,
                ajax: {
                    url: "{{ route('staff.staff_list_data_table') }}",
                    data: function(d) {
                        d.type = 'player';
                    }
                },
            ...
            $(document).on('click', '.approve_account', function(e) {
                ...
                $.ajax({
                    url: "{{ route('staff.approve_staff_account') }}",
                    method: 'POST',
                    data: {
            ...
            $(document).on('click', '.active-account-btn', async function(e) {
                if (await isConfirm({
                    ...
                    $.ajax({
                        url: "{{ route('staff.active-staff-account') }}",
                        method: 'POST',
                        data: {
                            'user_id': $(this).attr('data-id'),
                            'status': '1'
        ...
        $(document).on('click', '.edit-staff-btn', function() {
            let id = $(this).data('id');
            $.ajax({
                url: "/staff-dashboard/edit-staff/" + id,
                type: 'GET',
                success: function(response) {
                    if ($.trim(response) === "") {
        ...
        $(document).on('click', '.view-staff-btn', function() {
            let id = $(this).data('id');
            $.ajax({
                url: "/staff-dashboard/view-staff/" + id,
                type: 'GET',
                success: function(response) {
                    if ($.trim(response) === "") {
                        swal_error_popup("Staff data not found");
