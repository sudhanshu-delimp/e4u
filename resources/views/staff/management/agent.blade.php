...
@section('content')
    ...
    @php
        ...
        $addAccessEnabled = isset($addAccess['yesNo']) && $addAccess['yesNo'] == 'yes' ? true : false;
        $viewAccessEnabled = isset($viewAccess['yesNo']) && $viewAccess['yesNo'] == 'yes' ? true : false;
        $editAccessEnabled = isset($editAccess['yesNo']) && $editAccess['yesNo'] == 'yes' ? true : false;
    @endphp
    <!-- Content Wrapper -->
...
@push('script')
    ...
    <script>
        $(document).ready(function(e) {
            var table = $("#agent_data_table").DataTable({
                ...
                ajax: {
                    url: "{{ route('staff.agent_list_data_table') }}",
                    data: function(d) {
                        d.type = 'player';
                    }
                },
                ...
                var modal_html = `<div class="modal-dialog modal-dialog-centered" role="document">
                  <div class="modal-content basic-modal">
                     ...
                     <div class="modal-body">
                        <form name="update_agent" method="POST" action="{{ route('staff.update-agent') }}" enctype="multipart/form-data">
                           <div class="row">
            ...
            $(document).on('submit', 'form[name="update_agent"]', function(e) {
                ...
                $.ajax({
                    url: "{{ route('staff.check-agent-email') }}",
                    method: "POST",
                    data: formData,
                    contentType: false,
                    processData: false,
            ...
            $(document).on('click', '.approve_account', function(e) {
                ...
                $.ajax({
                    url: "{{ route('staff.approve-agent-account') }}",
                    method: 'POST',
                    data: {
            ...
            $(document).on('click', '.active-account-btn', async function(e) {
                ...
                if (await isConfirm({
                    ...
                    $.ajax({
                        url: "{{ route('staff.active-agent-account') }}",
                        method: 'POST',
                        data: {
                            'user_id': $(this).attr('data-id'),
                            'status': '1'
    ...
    <div style="font-family:Arial; max-width:100%; overflow:hidden; box-sizing:border-box;">
        ...
        <table style="width:100%; border-collapse:collapse; table-layout:fixed; font-size:14px;" border="1">
            ...
            swal_waiting_popup({
                'title': 'Generating the PDF. Please wait...'
            });

            $.ajax({
                url: "{{ route('staff.generate-agent-info-pdf') }}",
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
