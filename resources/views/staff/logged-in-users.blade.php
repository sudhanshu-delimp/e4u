            ...
            <div class="modal-content basic-modal modal-lg">
                ...
                <div class="modal-body" id="escortPopupModalBody">
                    ...
                    <div class="modal-footer">
                        <form action="{{ route('staff.print.logged.user.single-details') }}" method="post">
                            {{ csrf_field() }}
    ...
    <script type="text/javascript">
        ...
        function ajaxReload() {
            var table = $('#listings').DataTable({
                ...
                pageLength: 10,
                ajax: {
                    url: "{{ route('staff.get-logged-in-users-by-ajax') }}",
                    type: "GET",
        ...
        $(document).on('click', '#saveSuspendPfileInfo', function(e) {
            ...
            $.ajax({
                url: '{{ route("staff.logged-user-status-update", "_id") }}'.replace('_id',userId), // replace with your actual route
                method: 'GET',
        ...
        $(document).on('click', '.view-listing', function(e) {
            ...
            $.ajax({
                url: '{{ route("staff.current.single-list.dataTableListing") }}/' +
        ...
        function viewSingleLoggedUserAjaxRequest(userId) {
            $.ajax({
                url: '{{ route("staff.get-logged-in-single-user-detail-with-ajax", "_id") }}'.replace('_id',
