...
<table role="presentation" class="table table-striped" id="escort-media-table">
    <tbody class="files">
        ...
        <tr class="template-download fade image in">
            ...
            <td>
                <button class="btn btn-danger delete" data-type="DELETE" data-url="{{ route('agent.delete.media', $media->id) }}">
                ...
                @if($media->type == 0)
                    ...
                    @else
                    <button type="button" class="btn btn-primary mark-default" data-id="{{ $media->id }}" data-url="{{ route('agent.media.mark.default', $media->id) }}" data-toggle="tooltip" data-placement="top" title="Mark this as default image">
                        Default
                    </button>
                    @endif
                @endif
