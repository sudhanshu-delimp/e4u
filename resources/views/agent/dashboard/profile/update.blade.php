                    ...
                    <div class="row">
                        <div class="col-md-12 remove_padding_in_ph">
                            ...
                            @if(request()->segment(2) == "profile")
                                <form id="my_escort_profile" action="{{ route('agent.setting.profile',[request()->segment(3),request()->segment(4)] ) }}" method="post" enctype="multipart/form-data">
                            @else 
                                <form id="my_escort_profile" action="{{ route('agent.setting.profile',[0,request()->segment(3)] ) }}" method="post" enctype="multipart/form-data">
                            @endif
                                @csrf
                                <input type="hidden" id="uid" value="{{ $user->id }}">
                                <input type="hidden" name="user_startDate" id="user_startDate" value="{{ date('Y-m-d',strtotime($user->created_at)) }}">
...
<script>
    ...
    $('#state_id').change(function(){
        ...
        var url = "{{ route('agent.stateByCity',':id') }}";
       ...
       //console.log(url);
        $.ajax({
                ...
                success: function(data) {
                    // {{--var optionString = ' <option value="" selected="">-- Select cities--</option>';--}}
                    var optionString = '';
                    //console.log(data);
    ...
    // });
        $("body").on("submit","#my_escort_profile",function(e){
            ...
            //console.log(data);
            console.log("submit form 44", url);
                ...
                $.ajax({
                    ...
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    success: function (data) {
                        //console.log("data.my_data.url=",data.my_data.url);
                        ...
                        if(data.my_data.new_profile == 1){
                            if(data.my_data.change_amount == true) {
                                if(data.my_data.draft == 1) {
                                    ...
                                    var url = "{{ route('agent.poli.paymentUrl',':id')}}";
    ...
    $("body").on("click","#defaultImg",function(){  
        var uid = $("#uid").val();
        var url = "{{ route('agent.get.default.images',':id')}}";
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if(data.error == true){
                    ...
                    $("#imgx").attr('src',data.path[10]['path']);$("#mediaIdx").val(data.path[10]['id']);
    ...
    $(document).ready(function() {
        ...
        $("body").on("click","#update-about-me",function(){
