...
@section('content')
                                ...
                                <div class="tab-content">
                                    <div class="tab-pane fade in active show" id="tab1warning">
                                        <div class="table-responsive-xl">
                                            <table class="table" id="legBoxNotificationforEscortTable">
                        ...
                        <div class="row">
                            ...
                            <div id="templateSelect" style="display: none;" class="col-12 mb-3">
                                ...
                                <select id="template_name" name="template_name" class="form-control rounded-0">
                                    ...
                                    <option value="Only a few days left before I leave!">
                                        Only a few days left before I leave!</option>
                                    <option value="I am back, when can we catch up?">
                                        I am back, when can we catch up?</option>
                                    <option
                                        value="I am with my Playmate friend, lets get together!">
                                        I am with my Playmate friend, lets get together!
                                    </option>
                                    <option
                                        value="I have just updated my Profile, check it out!">
                                        I have just updated my Profile, check it out!</option>
                                    <option
                                        value="I am free now, let's catch up!">
                                        I am free now, let's catch up!</option>
                                    <option value="You are so special to me, 10% discourt for today only!">
                                        You are so special to me, 10% discourt for today only!
                                    </option>
                                </select>
                            </div>
    ...
    <div id="manage-route" data-scrf-token="{{ csrf_token() }}"
        ...
        data-pdf-download="{{ route('escort.legbox.notification.pdf.download', ['id' => '__ID__']) }}"
        data-legbox-notification-index="{{ route('escort.legbox.notification.index') }}"
        data-legbox-notification-status="{{ route('escort.legbox.notification.status', ['id' => '__ID__']) }}"
        data-legbox-notification-edit="{{ route('escort.legbox.notification.edit', ['id' => '__ID__']) }}"
        data-legbox-notification-update="{{ route('escort.legbox.notification.update', ['id' => '__ID__']) }}"
        data-legbox-notification-store="{{ route('escort.legbox.notification.store') }}"
        data-legbox-notification-show="{{ route('escort.legbox.notification.show', ['id' => '__ID__']) }}">
...
@push('script')
    ...
    <script>
        ...
        endpoint = {
            ...
            center_legbox_notification_status: mmRoot.data('legbox-notification-status'),
            center_legbox_notification_edit: mmRoot.data('legbox-notification-edit'),
            center_legbox_notification_store: mmRoot.data('legbox-notification-store'),
            center_legbox_notification_show: mmRoot.data('legbox-notification-show'),
            center_legbox_notification_index: mmRoot.data('legbox-legbox-notification-index'),
        }
        ...
        function ensureParsleyAndSubmit(form) {
            ...
            $.ajax({
                url: endpoint.center_legbox_notification_store,
                type: "POST",
                dataType: "json",
                headers: {
                ...
                error: function(xhr) {
                    ...
                    $('#success_form_html').html(
                        '<h4>' + msg + '</h4>' +
                        '<button type="button" class="btn-success-modal mt-3 shadow-none" data-bs-dismiss="modal">OK</button>'
                    );
                    $('#successModal').modal('show');
                }
            });
        ...
        var table = $("#legBoxNotificationforEscortTable").DataTable({
            language: {
            ...
            serverSide: true,
            ajax: {
                url: endpoint.center_legbox_notification_index,
                type: 'GET'
            },
            columns: [{
                    data: 'ref',
        ...
        $(document).on('click', '.js-view', function(e) {
            ...
            $.ajax({
                url: endpoint.center_legbox_notification_show.replace('__ID__', id),
                type: 'GET',
                success: function(response) {
                    if (response.status === true) {
        ...
        $(document).on('click', '.js-suspend, .js-publish, .js-remove', function(e) {
            ...
            if ($(this).hasClass('js-suspend')) {
                status = 'Suspended';
                confirmMsg = 'Are you sure you want to suspend this lebbox notification';
            } else if ($(this).hasClass('js-publish')) {
                status = 'Published';
                confirmMsg = 'Are you sure you want to publish this lebbox notification';
            } else if ($(this).hasClass('js-remove')) {
                status = 'Removed';
                confirmMsg = 'Are you sure you want to remove the lebbox notification';
            }
            ...
            body.off('click', '#confirmRemove').on('click', '#confirmRemove', function() {
                ...
                $.ajax({
                    url: endpoint.center_legbox_notification_status.replace('__ID__', id),
                    type: 'POST',
                    data: {
        ...
        $(document).on('click', '.js-edit', function(e) {
            ...
            $.ajax({
                url: endpoint.center_legbox_notification_edit.replace('__ID__', id),
                type: 'GET',
                success: function(response) {
                    if (response.status === true) {
                        let n = response.data;
