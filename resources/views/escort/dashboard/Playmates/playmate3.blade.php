...
<div class="container-fluid pl-3 pl-lg-5 pr-3 pr-lg-5">
   <div class="row">
      <div class="col-md-12">
         <div class="v-main-heading h3">My Playmates</div>
      </div>
      <div class="col-md-12 mt-4">
         <div class="card">
            <div class="card-body">
               ...
               <ol class="mb-0">
                  <li>By completing these settings, the information set out under My playmates will by
                     default appear in your Profile creator. You should check with the Escort they are still available as a playmate before creating a Profile. A Viewer will be able to view the playmate’s Profile from your Profile.
                  </li>
            ...
            <div class="card-body active-play border-0 pt-0 mt-1">
               <div class="at-lable  mt-0">
                  <h2 class="primery_color normal_heading">My Active Playmates</h2>
                  <ul class="results  mt-2 activePlaymate">
                     {{-- {{ dd(auth()->user()->playmates) }} --}}
                     @if(!is_null(auth()->user()->playmates))
                     @foreach(auth()->user()->playmates as $playmate)
                     <li id="rmlist_{{$playmate->id}}"> <a href="#">
                        <img src="{{ $playmate->DefaultImage ? asset($playmate->DefaultImage) : asset('assets/app/img/icons-profile.png') }}" class="img-profile rounded-circle playmats-img">{{$playmate->name}}</a>
...
<script>
   ...
   function formatEscortList (data) {
       ...
       return $('<span><img class="profile-user-img img-responsive img-circle img-profile rounded-circle small-round-fixed" src="'+data.text+'"> '+data.name+' || '+data.member_id+'</span><span class="playmates_id" value="'+data.id+'" data-path="'+data.text+'" data-name="'+data.name+'">Add</span>');
   ...
   function selectEscortList (data) {
        ...
        $(".activePlaymate").append("<li><a href='#'><img src='"+path+"' class='img-profile rounded-circle playmats-img'>&nbsp;&nbsp;"+name+"</a><span class='playmates_rmid' value="+id+">×</span></li>");
