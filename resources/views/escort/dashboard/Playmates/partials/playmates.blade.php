...
<script>
   ...
   function formatEscortList (data) {
       if(data.name != undefined) {
           ...
           return $('<span><img class="profile-user-img img-responsive img-circle img-profile rounded-circle small-round-fixed" src="'+path+'"> '+data.member_id+' - '+data.name+' </span><span class="playmates_id" value="'+data.id+'" data-path="'+data.text+'" data-name="'+data.name+'">Add</span>');
       }
   }
   ...
   $(document).click(function(){
    ...
    $('#search-playmate-input').on('select2:select', function (e) {
        e.preventDefault();
        console.log(e.params.data);
        var id = e.params.data.id;
        var name = e.params.data.name;
        var path = e.params.data.text;
        var member_id = e.params.data.member_id;
