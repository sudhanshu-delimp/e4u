...
@section('content')
                ...
                <div class="card collapse" id="notes" style="">
                    <div class="card-body">
                        ...
                        <ol>
                            ...
                            <li>Use the <a href="{{ route('escort.dashboard.agent-messages') }}"
    ...
    <div class="modal fade upload-modal" id="conversation_modal" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-scrollable modal-lg modal-dialog-centered" role="document"> {{-- NOTE:: use  modal-dialog-scrollable instead of modal-dialog to make body scrollable only --}}
            <div class="modal-content">
                {{-- {{ route('escort.upload.gallery') }} --}}
                <div class="modal-content border-0">
                    <div class="modal-header">
                        <span class="custom-pop-wrapper"><img src="/assets/app/img/history.png" class="custompopicon"
                                alt="cross">
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            method: "PUT",
                            dataType: "json",
                            url: "{{ route('support-ticket.withdraw', ':id') }}".replace(
                                ':id', ticketId),
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                            },
                            success: function(data) {
                                if (data.status == "success") {
                                    cancelTicketBtn.closest('tr').find('.status').data(
                                        'status-id', 4).html('Withdrawn');
                                    cancelTicketBtn.closest('tr').find('.cancelTicket')
                                        .remove();
                                    Swal.fire({
                                        icon: 'success',
                                        title: 'Withdrawn',
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status == 'Resolved' || data.status == 'Withdrawn') {
                        ...
                        resolved =
                            `<div class="col-sm-12 text-center complete_ticket mt-3" style="font-weight: 700; font-size: 20px;color: green;"> ${message}</div>`
                    }
                    ...
                    var html = '<div class="col-sm-6 conversation"> </div>' +
                        ...
                        '       <p>' + data.message + '</p>' +
                        '   </div>' +
                        '       <span class="message_time"> Member ID: ' + data.user.member_id + ',  ' +
                        date_time_format(data.created_on) + '</span>' +
                        '</div>';
                    $(data.conversations).each(function(index, conversation) {
                        if (conversation.admin_id) {
                            html +=
                                ...
                                '   </div>' +
                                '       <span class="message_time"> Member ID: ' + conversation
                                .user_from_admin.member_id + ',   ' + date_time_format(conversation
                                    .date_time) + '</span>' +
                                '</div>' +
                                // '<div class="col-sm-6 conversation"> </div>' +
                                '<div class="col-sm-6 conversation"> </div>';
                            ...
                            html += '<div class="col-sm-6 conversation"> </div>' +
                                ...
                                '   </div>' +
                                '       <span class="message_time">Member ID: ' + conversation
                                .user_from_user.user_id + ',   ' + date_time_format(conversation
                                    .date_time) + '</span>' +
                                '</div>';
                        }
                    });
