        ...
        @endif
            ...
            <div class="mc_card_header">
                ...
                <span class="mc_title">{{$listing->business_name}}</span>
                 @if(auth()->user())
                    @if(auth()->user()->type == 0)
                        <span class="add_to_favrate sm_fav_icon @if(in_array($listing->id,$logedInUpser->massageCenterLegBox->pluck('id')->toArray())){{'null'}}@else{{'fill'}}@endif custom--favourite" id="legboxIdList_{{$listing->id}}"  data-massageId="{{$listing->id}}" data-userId="{{ auth()->user() ? auth()->user()->id : 'NA' }}" data-name="{{$listing->business_name}} ">
