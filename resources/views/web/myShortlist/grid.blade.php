...
<div class="ec_card brb--text">
    ...
    <div class="ec_card_header">
        ...
        @if (auth()->user())
            @if (auth()->user()->type == 0)
                <span
                    class="add_to_favrate @if (in_array($escort->id, $user_type->myLegBox->pluck('id')->toArray())) {{ 'null' }}@else{{ 'fill' }} @endif"
            ...
            @else
                <span class="add_to_favrate grid-custom-favourite"><i class="fa fa-heart-o" aria-hidden="true"></i>
                    <span class="custom-heart-text">Add to My Legbox</span></span>
            @endif
        @else
            <span class="add_to_favrate grid-custom-favourite" data-escortId="{{ $escort->id }}"><i
                    class="fa fa-heart-o" aria-hidden="true"></i> <span class="custom-heart-text">Add to My
                    Legbox</span></span>
        @endif
    </div>

    <a class="ec_card_link"
        href="{{ route('profile.description', [$escort->id, $escort->city_id, $escort->membership]) }}?grid">
        <div class="ec_profile_img">

            <img class="card-img-top"
                src="{{ $escort->default_image ? $escort->default_image : asset('assets/app/img/service-provider/Frame-408.png') }}"
                alt="Card image cap">
        ...
        <div class="ec_card_content">
            <div class="items">
                <span class="title">{{ $escort->city ? $escort->city->name : '' }}</span>
    ...
    <div class="ec_card_footer wishlist_footer">
        ...
        @else
            <a href="javascript:void(0)" class="short-list shortlist myescort_{{ $escort->id }}"
                ...
                data-escortId="{{ $escort->id }}" data-userId="{{ $viewerAuth ? $viewerAuth->id : 'NA' }}">
                @if (!empty($escortId))
