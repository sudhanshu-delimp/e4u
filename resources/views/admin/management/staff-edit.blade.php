<form name="add_staff" method="POST" action="{{ route('admin.store-staff') }}" enctype="multipart/form-data">
    <div class="row">
        ...
        <div class="col-6 mb-3">
            ...
            <input type="text" class="form-control rounded-0" placeholder="Full Name" name="name" id="name" value="{{$staff->name}}">
            <span class="text-danger error-name"></span>
        </div>
        <div class="col-6 mb-3">
            <input type="text" class="form-control rounded-0" placeholder="Address" name="address" id="address" value="{{$staff->staff_detail->address}}">
            <span class="text-danger error-address"></span>
        </div>
        <div class="col-6 mb-3">
            <input type="text" class="form-control rounded-0" placeholder="Phone" name="phone" id="phone" value="{{$staff->phone}}">
        ...
        <div class="col-6 mb-3">
            <select class="form-control" name="gender" id="gender">
                ...
                @foreach (config('escorts.profile.genders') as $key => $gender)
                    <option value="{{ $key }}" {{$staff->gender == $key ? 'selected' : ""}}>{{ $gender }}</option>
                @endforeach
            </select>
        ...
        <div class="col-6 mb-3">
            <input type="text" name="kin_mobile" id="kin_mobile" class="form-control rounded-0" placeholder="Mobile" value="{{$staff->staff_detail->kin_mobile}}">
            <span class="text-danger error-kin_mobile"></span>
        </div>
        <div class="col-6 mb-3">
            <input type="email" name="kin_email" class="form-control rounded-0" placeholder="Email (optional)" value="{{$staff->staff_detail->kin_email}}">
        ...
        <div class="col-6 mb-3">
            <select class="form-control rounded-0" name="security_level" id="security_level">
                ...
                @foreach (config('staff.security_level') as $seckey => $secLevel)
                    <option value="{{ $seckey }}" {{$staff->staff_detail->security_level == $seckey ? "selected": ""}}>{{ $secLevel }}</option>
                @endforeach
            </select>
        ...
        <div class="col-6 mb-3">
            <select class="form-control rounded-0" name="employment_status" id="employment_status">
                ...
                @foreach (config('staff.employment_status') as $empkey => $empStatus)
                    <option value="{{ $empkey }}" {{$staff->staff_detail->employment_status == $empkey ? "selected": ""}}>{{ $empStatus }}</option>
                @endforeach
            </select>
        ...
        <div class="col-6 mb-3">
            <select class="form-control rounded-0" name="employment_agreement" id="employment_agreement">
                ...
                <option value="yes" {{$staff->staff_detail->employment_agreement == "yes" ? "selected": ""}}>Yes</option>
                <option value="no" {{$staff->staff_detail->employment_agreement == "no" ? "selected": ""}}>No</option>
            </select>
        ...
        <div class="col-4 mb-3">
            <select class="form-control rounded-0" name="building_access_code" id="building_access_code">
                <option value="">Access Code Provided?</option>
                 <option value="yes" {{$staff->staff_detail->building_access_code == "yes" ? "selected": ""}}>Yes</option>
                <option value="no" {{$staff->staff_detail->building_access_code == "no" ? "selected": ""}}>No</option>
            </select>
