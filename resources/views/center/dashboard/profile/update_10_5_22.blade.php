                ...
                <div class="col-md-12">
                   ...
                   <div class="manage_process_bar_padding">
                      <div class="progress define_process_bar_width">
                         <div class="progress-bar define_process_bar_color" role="progressbar" style="width: 10%" aria-valuenow="20" aria-valuemin="0" aria-valuemax="100">
                         </div>
                      </div>
                   </div>
                </div>
...
<script>
        ...
        let editor2 = CKEDITOR.replace(textarea2);
...
///////////////clear reset ////////////////////  
    ...
    $('body').on('change','#service_id_one', function(){
        ...
        if(selectedIdOne){
            $("#selected_service_one").append(" <li id='hideenclassOne_"+ selectedIdOne+"'><div class='my_service_anal' ><span class='dollar-sign'>"+getNameOne+"</span><input type='number' class='dollar-before input_border' name='price[]' placeholder='' min='0' oninput='this.value = Math.abs(this.value)' step=10 max=200><input type='hidden' name='service_id[]' value="+ selectedIdOne +" placeholder=''><input type='hidden' name='category_id[]' value='1'><span><i class='fas fa-times-circle akh1' data-sname='"+getNameOne+"' data-val="+ selectedIdOne+"  id='id_"+ selectedIdOne+"' value="+selectedIdOne+"></i></span></div></li> ");
    ...
    $('body').on('change','#service_id_two', function(){
        ...
        if(selectedIdTwo){
            $("#selected_service_two").append(" <li id='hideenclassTwo_"+ getNameTwo+"'><div class='my_service_anal hideenclassTwo"+selectedIdTwo+"'><span class='dollar-sign'>"+getNameTwo+"</span><input type='number' class='dollar-before input_border' name='price[]' placeholder='' min='0' oninput='this.value = Math.abs(this.value)' step=10 max=200><input type='hidden' name='service_id[]' value="+ selectedIdTwo +" placeholder=''><input type='hidden' name='category_id[]' value='2'><span><i class='fas fa-times-circle akh2'  data-sname='"+getNameTwo+"' data-val="+ selectedIdTwo+"  id='id_"+ selectedIdTwo+"' value="+selectedIdTwo+"></i></span></div></li> ");
            ...
            $('a[data-toggle="tab"]').on('show.bs.tab', function (e) {
               ...
               // e.target // newly activated tab
                //e.relatedTarget // previous active tab
                    if($('#my_massage_profile').parsley().validate({group: 'block-0',force: true})){
                        e.target
                        if(e.target.id == "profile-tab") {
                            $('.define_process_bar_color').attr('style','width :30%');//.percent
                            $('#percent').html('40%');
                        } else if(e.target.id == "massuers-tab") {
                            $('.define_process_bar_color').attr('style','width :50%');//.percent
                            $('#percent').html('60%');
                        } else if(e.target.id == "contact-tab") {
                            $('.define_process_bar_color').attr('style','width :70%');//.percent
                            $('#percent').html('80%');
                        } 
                            ...
                            $("#poli_payment").click(function(e){
                                ...
                                $.ajax({
                                    ...
                                    //headers: "Authorization: Basic {{ config('escorts.poli_payment_authorization_code')}}",
                                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                                    success: function (data) {
                                        console.log(data.json.NavigateURL);
                                        // $("#iframeContainer").show();
                                        // $("#iframeContainer1").show();
                                        // $("#poliframe").attr('src',data.json.NavigateURL);
                                        window.open(data.json.NavigateURL, "_blank")
                            ...
                            $('.define_process_bar_color').attr('style','width :10%');//.percent
                            $('#percent').html('20%');
                        }
                    } else {
