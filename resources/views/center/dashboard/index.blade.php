...
<div class="container-fluid pl-3 pl-lg-5 pr-3 pr-lg-5">
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="alert text-center text-white bg-second" role="alert" id="notification-bar" style="display: none;"> </div>
        </div>
    </div>
    <div class="row">
...
<script>
    ...
    function loadNotification() {
        $.ajax({
            url: "{{route('center.active.notification')}}",
            method: 'GET',
            success: function(data) {
                console.log(data.data);
                if (data.status == true) {
                    $('#notification-bar').html(data.data ? data.data : '').show();
                } else {
    ...
    $(document).ready(function() {
        loadNotification();
        setInterval(loadNotification, 300000 * 12);
    });
</script>
@endsection