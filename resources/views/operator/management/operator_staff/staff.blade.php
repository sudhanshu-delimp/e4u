    ...
    <script>
        $(document).ready(function() {
            var table = $("#staff_data_table").DataTable({
                ...
                ajax: {
                    url: "{{ route('operator.operator.staff_list_data_table') }}",
                    data: function(d) {
                        d.type = 'player';
                    }
                },
            ...
            $(document).on('submit', 'form[name="add_staff"]', function(e) {
                ...
                $.ajax({
                    url: "{{ route('operator.operator.add-staff') }}",
                    method: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
            ...
            $(document).on('click', '.account-suspend-btn', async function(e) {
                if (await isConfirm({
                    ...
                    ajaxRequest({
                        url: "{{ route('operator.operator.suspend-staff') }}",
                        method: 'POST',
                        data: {
                            id: $(this).data('id'),
            ...
            $(document).on('click', '.approve_account', async function(e) {
                if (await isConfirm({
                    ...
                    $.ajax({
                        url: "{{ route('operator.operator.approve_staff_account') }}",
                        method: 'POST',
                        data: {
            ...
            $(document).on('click', '.active-account-btn', async function(e) {
                if (await isConfirm({
                    ...
                    $.ajax({
                        url: "{{ route('operator.operator.active-staff-account') }}",
                        method: 'POST',
                        data: {
                            'user_id': $(this).attr('data-id'),
                            'status': '1'
        ...
        $(document).on('click', '.edit-staff-btn', function() {
            let id = $(this).data('id');
            $.ajax({
                url: "/operator-dashboard/edit-operator-staff/" + id,
                type: 'GET',
                success: function(response) {
                    if ($.trim(response) === "") {
        ...
        $(document).on('click', '.view-staff-btn', function() {
            let id = $(this).data('id');
            $.ajax({
                url: "/operator-dashboard/view-operator-staff/" + id,
                type: 'GET',
                success: function(response) {
                    if ($.trim(response) === "") {
