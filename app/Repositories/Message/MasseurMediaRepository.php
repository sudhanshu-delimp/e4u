...
class MasseurMediaRepository extends BaseRepository implements MasseurMediaInterface
{
    protected $media;

    public function __construct(MasseurMedia $media)
    {
        $this->model = $media;
    }
  ...
  public function with_Or_withoutPosition($user_id, $page_token, $position = null)
    {

        $result = $this->model
            ->where('user_id',$user_id)
            ->where('masseur_token_id',$page_token)
