...
class AgentEscortRepository extends BaseRepository implements AgentEscortInterface
{
    ...
    {   
        foreach($result as $key => $item) {
            ...
            $item->action = '<div class="dropdown no-arrow"> <a class="dropdown-toggle" href="" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <i class="fas fa-ellipsis fa-ellipsis-v fa-sm fa-fw text-gray-400"></i> </a> <div class="dot-dropdown dropdown-menu dropdown-menu-right shadow animated--fade-in" aria-labelledby="dropdownMenuLink"> <a class="dropdown-item d-flex align-items-center justify-content-start gap-10" href="../escort-profile/'.$item->id.'" data-id="'.$item->id.'"><i class="fa fa-eye"></i> view</a> <div class="dropdown-divider"></div><a class="dropdown-item d-flex align-items-center justify-content-start gap-10" href="profile/'.$item->id.'" data-id="'.$item->id.'" data-name="'.$item->name.'" data-category="'.($item->id).'"> <i class="fa fa-pen"></i> Edit</a> <div class="dropdown-divider"></div><a class="dropdown-item d-flex align-items-center justify-content-start gap-10" href="#" data-id="'.$item->id.'" data-name="'.$item->name.'"  data-city="'.($item->city ? $item->city->name : null).'" data-url="playmates/'.$item->id.'" data-toggle="modal" data-target="#play-mates-modal"><i class="fa fa-users"></i> Playmates</a> <div class="dropdown-divider"></div><a class="dropdown-item d-flex align-items-center justify-content-start gap-10 delete-center" href="delete-profile/'.$item->id.'" data-id="'.$item->id.'"> <i class="fa fa-trash"></i> Delete </a> </div></div>';
		}
        return $result;
    }
