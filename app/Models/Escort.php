...
class Escort extends Model
{
    ...
    public function getCovidreportAttribute($value)
    {
        switch ($value) {
            case (1):
                return "Vaccinated, not up to date";
                break;
            case (2):
                return "Vaccinated, up to date";
                break;
            case (3):
    ...
    {
        return optional($this->gallary->first())->path
            ? url($this->gallary->first()->path)
            : null;
    }

    public function oneHourDuration()
    {
