    ...
    public function process(array $payload)
    {
        ...
        foreach (['utc_start_time', 'utc_end_time'] as $key) {
            if (!empty($payload[$key])) {
                $payload[$key] = \Carbon\Carbon::parse($payload[$key])->toDateTimeString();
            }
        }
        ...
        {
            $insert = json_decode(json_encode($metadata['insert']), true); 
            ...
            if (!empty($insert['loyalty_amount']) && $insert['loyalty_amount'] > 0) {
                if ($rate != "" && $rate <= $insert['loyalty_amount']) {
                    ...
                    $user->wallet->wallet->decrement('earn_days', $loyalty_day);
                }
            }
