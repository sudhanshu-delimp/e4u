...
class ReportingController extends BaseController
{
    ...
    //         if ($user->status == $request->status && !$request->filled('reject_reason')) {
    ...
    //         if ($request->status == 7) {
    //             if($user->status == 6){
    //                 $user_prev_status = $user->status;  
    //             }
    //             // Update reason if provided, even if status didn't change
    //             if ($request->filled('reject_reason')) {
    //                 $user->rejection_reason = $request->reject_reason;
    //             }else{
    //                 $user->rejection_reason = null;
    //             }
    //         } else {
    //             // Clear reason when status is not rejected
    //             $user->rejection_reason = null;
    //         }

    //         $user->save();
    //         $user->pre_status =  $user_prev_status;
    //         dd($user);
    //         // Status messages
    //         $statusInfo = [
    ...
    //         // Send email except when blocked (status 4)
    //         if ($request->status != 4) {
    //             Mail::to($user->email)
    //                 ->queue(new UserStatusUpdatedMail($user, $statusName, $statusMessage));
    //         }
