...
class ArchivesController extends Controller
{
     ...
     */
    public function __construct(CityInterface $city, StateInterface $state, UserInterface $user, EscortInterface $escort, AvailabilityInterface $availability,  ServiceInterface $service, DurationInterface $duration, EscortMediaInterface $media)
    {
        $this->escort = $escort;
    ...
    public function profileStates($id = null)
    {
        $user = auth()->user();
        $states = $this->state->allByCountryId();
        $stateId = [];
        foreach($states as $state) {
            $stateId[] = $state->id;
        }
        //dd($stateId);
        //dd($user->escorts->whereIn('state_id',$stateId));
        $escorts = $user->escorts->whereIn('state_id',$stateId);
        return view('escort.dashboard.archives.archive-view-profiles');
    }
    public function profileStatesList($stateId)
    {
        $user = auth()->user();
        ...
        //     $stateId[] = $state->id;
        // }
        //$cityName = config("escorts.profile.cities.$cityId");
        //dd($stateId);
        //dd($user->escorts->whereIn('state_id',$stateId));
        $escorts = $user->escorts->whereIn('state_id',$stateId);
        return view('escort.dashboard.archives.archive-view-profiles-list',compact('stateId'));
    }
    // public function stateDataTable($cityId)
    public function stateDataTable($stateId)
    {
       // dd($stateId);
