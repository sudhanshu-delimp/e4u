...
$(document).on('click', '#close_change', function () {
    ...
    console.log("No clicked → restoring status");
...
function getMediaByIdAndStatusShow(media_id, position) {
    ...
    let currentImg = $("#img" + position).attr('src');

    let srcArray = $(".upld-img").map(function () {
        return $(this).attr("src");
    }).get();

    let duplicateCount = srcArray.filter(src => src === currentImg).length;
